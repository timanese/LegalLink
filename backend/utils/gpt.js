...
Please only give the grade without an explanation`;
  ...
  try {
    ...
    const gradeExplanationPrompt = `Using this grade that was previously generated by you '${gradeResponse.data.choices[0].message.content.trim()}',
    please explain this grade for the following legal case, as if you are a lawyer, on a scale from 1 to 10, 
...
Case Description: '${caseDescription}'`;
    ...
    return {
      ...
      MNMProbability:
        MNMProbabilityResponse.data.choices[0].message.content.trim(),
      generatedCaseDescription:
        generatedCaseDescriptionResponse.data.choices[0].message.content.trim(),
    };
  } catch (error) {
...
`;
    ...
    return {
      ...
      generatedCaseDescription:
        generatedCaseDescriptionResponse.data.choices[0].message.content.trim(),
      reevaluateCase:
        reevaluationResponse.data.choices[0].message.content.trim(),
      reevaluationExplanation:
        reevaluationExplanationResponse.data.choices[0].message.content.trim(),
    };
  } catch (error) {
    console.log("Error generating text:", error);
  }
